...
func TestEffectiveOnDeathDefault(t *testing.T) {
	...
	for _, want := range []string{`"$GC_SESSION_ID" "$GC_SESSION_NAME" "$GC_ALIAS" "myrig/dog"`, `--status=in_progress`, `--assignee "" --status open`} {
...
func TestEffectiveOnDeathFixedAgent(t *testing.T) {
	a := Agent{Name: "mayor"}
	got := a.EffectiveOnDeath()
	for _, want := range []string{`"$GC_SESSION_ID" "$GC_SESSION_NAME" "$GC_ALIAS" "mayor"`, "--status=in_progress", `--assignee "" --status open`} {
		if !strings.Contains(got, want) {
			t.Errorf("EffectiveOnDeath() = %q, want %q", got, want)
		}
	}
...
func TestEffectiveOnBootDefault(t *testing.T) {
	...
	if strings.Contains(got, `--assignee ""`) {
		t.Errorf("EffectiveOnBoot() = %q, should not clear live assigned work by default", got)
	}
}
