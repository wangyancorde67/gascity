...
// Regression test for #742: with a file-backed store, an always-on named
// session must recover only the canonical named bead after tmux death.
// A leaked duplicate open bead for the same backing template must not be
// pulled back into desired state and re-woken alongside the named session.
func TestReconcileSessionBeads_FileStoreAlwaysNamedRecoversWithLeakedDuplicateOpenBead(t *testing.T) {
	...
	gotCanonical, err := store.Get(canonical.ID)
	if err != nil {
		t.Fatalf("Get(canonical): %v", err)
	}
	if gotCanonical.Status != "open" {
		t.Fatalf("canonical bead status = %q, want open", gotCanonical.Status)
	}
	if gotCanonical.Metadata["session_name"] != sessionName {
		t.Fatalf("canonical session_name = %q, want %q", gotCanonical.Metadata["session_name"], sessionName)
	}
}
